...
class Dtis(nn.Module):
    ...
    def forward(self, inputs, proteins):
        ...
        """Protein vector with pre-trained Transformer."""
        with torch.no_grad():
            results = self.esmmodel(proteins, repr_layers=[6])
        token_representations = results["representations"][6]
        protein_vector = token_representations[:, 1:, :]
        protein_vector_bert = torch.squeeze(protein_vector, 1)
        ...
        if (compound_vector_bert.size()[1] + protein_vector_bert.size()[1]) <= 512:
            ...
            token_type_ids = [0 for i in range(compound_vector_bert.size()[1])] + [1 for i in range(protein_vector_bert.size()[1])]
        else:
        ...
        outputs = self.bert(input_ids=None, token_type_ids=segments_tensors, attention_mask=None, inputs_embeds=cat_cp_vector)
        ...
        for j in range(self.layer_output):
            coa_vector = torch.tanh(self.W_out[j](coa_vector))

        """Concatenate the above two vectors and output the interaction."""
        #
        CLS_bert = self.lin(CLS_bert)
        CLS_bert = self.lin64(CLS_bert)
        cate_final_vector = torch.tanh(coa_vector + 0.3 * CLS_bert)

        interaction = self.W_interaction(cate_final_vector)
        #
        return interaction
